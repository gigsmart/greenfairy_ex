defmodule GreenFairy.Relay.Node do
  @moduledoc """
  ...
  defmacro __using__(opts \\ []) do
    quote do
      ...
      @before_compile GreenFairy.Relay.Node
    end
  end
      ...
      object :relay_node_queries do
        ...
        field :node, :node do
          ...
          resolve fn _, %{id: global_id}, resolution ->
            GreenFairy.Relay.Node.resolve_node(
              global_id,
              resolution,
              @__relay_node_opts__
            )
          ...
          resolve fn _, %{ids: global_ids}, resolution ->
            results =
              Enum.map(global_ids, fn global_id ->
                case GreenFairy.Relay.Node.resolve_node(
                       global_id,
                       resolution,
                       @__relay_node_opts__
                     ) do
  ...
  defp get_type_module(%{__private__: private}) do
    Keyword.get(private, :__green_fairy_module__)
  end
  ...
  defp fetch_node(type_module, local_id, context, opts) do
    cond do
      ...
      function_exported?(type_module, :__green_fairy_struct__, 0) ->
        struct = type_module.__green_fairy_struct__()
