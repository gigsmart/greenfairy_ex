defmodule GreenFairy.CQL.Scalars.ArrayInteger.Postgres do
  @moduledoc "PostgreSQL native integer array operators"
  ...
  def operator_input do
    {[
       ...
       :_is_empty,
       :_is_null
     ], :integer, "Operators for integer array fields"}
  end
  ...
  defp apply_excludes(query, field, value, nil) do
    where(query, [q], fragment("NOT (? = ANY(?))", ^value, field(q, ^field)))
  end

  defp apply_excludes(query, field, value, binding) do
    where(query, [{^binding, a}], fragment("NOT (? = ANY(?))", ^value, field(a, ^field)))
  end

  defp apply_includes_all(query, field, values, nil) when is_list(values) do
    where(query, [q], fragment("? @> ?::integer[]", field(q, ^field), ^values))
  end

  defp apply_includes_all(query, field, values, binding) when is_list(values) do
    where(query, [{^binding, a}], fragment("? @> ?::integer[]", field(a, ^field), ^values))
  end

  defp apply_excludes_all(query, field, values, nil) when is_list(values) do
    where(query, [q], fragment("NOT (? && ?::integer[])", field(q, ^field), ^values))
  end

  defp apply_excludes_all(query, field, values, binding) when is_list(values) do
    where(query, [{^binding, a}], fragment("NOT (? && ?::integer[])", field(a, ^field), ^values))
  end
  ...
  defp apply_is_empty(query, field, true, nil) do
    where(
      ...
      [q],
      fragment(
        "(array_length(?, 1) IS NULL OR ? = ARRAY[]::integer[])",
        field(q, ^field),
        field(q, ^field)
      )
    )
